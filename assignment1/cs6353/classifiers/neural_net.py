...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #print('W2',W2.shape)
    #print('X', X.shape)
    h1 = np.maximum(0,np.dot(X, W1) + b1) # calculate first hidden layer activations (4x1)
    ...
    #print(scores.shape)
    scores_sums = np.sum(expscores,axis=1)
    correct_class_scores = scores[range(num_train), y]
    #print(scores_sums.shape)
    sloss = -correct_class_scores + np.log(scores_sums)
    ...
    #############################################################################
    dtotal_loss = 1/num_train
    dreg_loss = reg
    dscores = expscores/(scores_sums.reshape(num_train, 1))
